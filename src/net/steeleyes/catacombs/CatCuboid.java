...
public class CatCuboid extends Cuboid {
	...
	public CatCuboid(World world, int xl, int yl, int zl, int xh, int yh,
			int zh, Type type) {
		...
		for (int y = yl; y <= yh; y++) {
			for (int x = xl; x <= xh; x++) {
				for (int z = zl; z <= zh; z++) {
					...
					info.add((x - top.x) + "," + (y - top.y) + ","
							+ (z - top.z) + "," + blk.getTypeId() + ","
							+ blk.getData());
				}
			}
		}
	...
	// TODO: need to figure out how to expand the table without loosing
	...
	public CatMat guessMajorMat(int roofDepth) {
		...
		int cnt = 0;
		if (debug == 1)
			System.out.println("[Catacombs] roofDepth=" + roofDepth);
			...
			for (int z = zl; z <= zh; z++) {
				...
				if (!mat.is(Material.AIR) && !mat.is(Material.TORCH)) {
					...
					if (near_air) {
						if (debug == 1)
							System.out
									.println("[Catacombs]     near air Block mat="
											+ mat + " blk=" + blk.getType());
						if (!mat.equals(last)) {
							if (cnt > best) {
								best_mat.get(last);
		...
		if (debug == 1)
			System.out.println("[Catacombs] final guess mat = " + best_mat
					+ " best=" + best);
		// debug++;
		return best_mat;
	}
	...
	public void unrender(BlockChangeHandler handler, Boolean emptyChest,
		...
		if (type == Type.LEVEL)
			...
			CatCuboid floor = new CatCuboid(world, xl, yl, zl, xh,
					yh - num_air, zh);
			// Go one higher to cover anything (torches typically) on the hut
			// roof.
			CatCuboid upper = new CatCuboid(world, xl, yh - num_air + 1, zl,
					xh, yh + 1, zh);
	...
	public void removeGlow(CatMat mat, int roofDepth) {
		for (int x = xl; x <= xh; x++) {
			for (int z = zl; z <= zh; z++) {
				...
				Block blk = world.getBlockAt(x, y, z);
				Block other = blk.getRelative(BlockFace.DOWN, 1);
				if (blk.getType() == Material.GLOWSTONE
						&& other.getType() == Material.AIR) {
					mat.setBlock(blk);
				}
			}
		}
			...
			for (int z = zl; z <= zh; z++) {
				for (int y = yl; y <= yh; y++) {
					...
					if (blk.getType() == Material.WOODEN_DOOR
							|| blk.getType() == Material.IRON_DOOR_BLOCK) {
				...
				for (int y = yl; y <= yh; y++) {
					...
					if (blk.getType() == Material.CHEST) {
						if (type == Type.LEVEL) { // Leave chests in the hut
							...
							chest.getInventory().clear(); // Clear and refill
															// chests inside
							if (isBigChest(blk)) {
						...
						cont.getInventory().clear(); // Clear and refill chests
														// inside
						CatLoot.fillChest(config, cont.getInventory(),
								config.TrapList());
					}
				}
			}
	...
	// Issues with the secret door piston creation in earlier versions of
	// Catacombs
			...
			for (int z = zl; z <= zh; z++) {
				for (int y = yl; y <= yh; y++) {
					...
					if (blk.getType() == Material.PISTON_STICKY_BASE
						...
						above.setTypeIdAndData(
								Material.PISTON_EXTENSION.getId(), (byte) 9,
								false);
						blk.setTypeIdAndData(
								Material.PISTON_STICKY_BASE.getId(), (byte) 9,
								false);
						cnt++;
					} else if (blk.getType() == Material.PISTON_STICKY_BASE
							&& below.getType() != Material.REDSTONE_TORCH_ON
						...
						above.setTypeIdAndData(above2.getType().getId(),
								above2.getData(), false);
						above2.setType(Material.AIR);
						blk.setTypeIdAndData(
								Material.PISTON_STICKY_BASE.getId(), (byte) 1,
								false);
						cnt++;
					}
				}
			...
			for (int z = zl; z <= zh; z++) {
				for (int y = yl; y <= yh; y++) {
					...
					if ((mat == Material.WOODEN_DOOR && above.getType() == Material.WOODEN_DOOR)
							|| (mat == Material.IRON_DOOR_BLOCK && above
			...
			for (int z = zl; z <= zh; z++) {
				for (int y = yl; y <= yh; y++) {
					...
					if (mat == Material.WOODEN_DOOR
						...
						blk.setTypeIdAndData(Material.IRON_DOOR_BLOCK.getId(),
								blk_b, false);
						above.setTypeIdAndData(
								Material.IRON_DOOR_BLOCK.getId(), above_b,
								false);
						cnt++;
					}
				}
	...
	private byte getDoorLowerCode(Block blk) {
		...
		if (CatUtils.isSolid(e))
			return 0;
		if (CatUtils.isSolid(w))
			return 0;
		if (CatUtils.isSolid(s))
			return 1;
		if (CatUtils.isSolid(n))
			return 1;
		return 0;
	}
	...
	public Boolean isBigChest(Block blk) {
		...
		for (BlockFace dir : Arrays.asList(BlockFace.NORTH, BlockFace.EAST,
	...
	public void setCube(BlockChangeHandler handler, Material mat,
			Boolean emptyChest) {
		for (int x = xl; x <= xh; x++) {
			for (int z = zl; z <= zh; z++) {
				for (int y = yl; y <= yh; y++) {
					...
					if (before == Material.LADDER || before == Material.TORCH
						...
						if (md instanceof Bed) {
							Bed bed = (Bed) md;
							if (bed.isHeadOfBed()) {
								Block foot = blk.getRelative(bed.getFacing()
										.getOppositeFace());
